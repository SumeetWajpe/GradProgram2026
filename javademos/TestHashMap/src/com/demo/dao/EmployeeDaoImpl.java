...
public class EmployeeDaoImpl implements EmployeeDao{
	...
	public List<Employee> findByName(String nm) {
		...
		Set<Integer> kset=hmap.keySet();  //{12,13,14,145}
		for(Integer e:kset) {
			if(hmap.get(e).getEname().equals(nm)) {
				elist.add(hmap.get(e));
			}
		}
        Set<Employee> hset=(Set<Employee>)hmap.values();
	    //List<Employee> elist=kset.stream()
        //.map(key->hmap.get(key))
        //filter(emp->emp.getEname().equals(nm))
        //   .collect(Collectors.toList());

        //another way
        // List<Employee> elist= hset .filter(emp->emp.getEname().equals(nm))
        //   .collect(Collectors.toList());
	    if(elist.size()>0)
			return elist;
		return null;
	...
	public boolean removeById(int id) {
		Employee e=hmap.remove(id);  ///remove key,value pair and return value
		if(e!=null)
			return true;
		return false;
